...
- ⬜ **Slice 5 — Email channel** — `EmailSender` abstraction (dev Mailpit / prod Resend), `NotificationEmailComposer` (recipient-language i18n templates), preference gating, throttle/digest for non-critical.
...
### Gaps to fill
- ❌ **No system/transactional email** (no `JavaMailSender`, no SMTP/Resend config). Slice 5 introduces it behind an abstraction, aligned with the planned GCP deployment (Mailpit dev / Resend prod).
