...
class RemoveLabelNodeProcessorTest {

    @Mock private RemoveLabelNodeExecutor removeLabelNodeExecutor;

    private RemoveLabelNodeProcessor processor;
    private UUID userId;
    private Email email;

    @BeforeEach
    void setUp() {
        processor = new RemoveLabelNodeProcessor(removeLabelNodeExecutor, new ObjectMapper());
        userId = UUID.randomUUID();
        email = createEmail(UUID.randomUUID());
    }

    @Test
    void getNodeType_returnsRemoveLabel() {
        assertThat(processor.getNodeType()).isEqualTo(NodeType.REMOVE_LABEL);
    }

    @Test
    void process_dryRun_returnsSimulated() {
        Map<String, Object> detail = Map.of("removedIds", "cat1", "reason", "dry-run");
        when(removeLabelNodeExecutor.execute(any(Email.class), any(), eq(true))).thenReturn(detail);
    ...
    void process_realRun_returnsExecuted() {
        Map<String, Object> detail = Map.of("removedIds", "cat1");
        when(removeLabelNodeExecutor.execute(any(Email.class), any(), eq(false))).thenReturn(detail);
    ...
    void process_invalidConfig_returnsError() {
        AutomationNode node = buildNode("not json");
        ExecutionContext context = new ExecutionContext(email, null, false);
    ...
    private AutomationNode buildNode(String config) {
        return AutomationNode.builder()
                .id(UUID.randomUUID())
                .nodeType(NodeType.REMOVE_LABEL)
                .label("Remove Label")
                .config(config)
